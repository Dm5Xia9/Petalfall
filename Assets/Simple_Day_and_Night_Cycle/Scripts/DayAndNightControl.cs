...
public class DayAndNightControl : MonoBehaviour {
	...
	public static DateTime Now => new DateTime(DateTime.Now.Year, DateTime.Now.Month, (Instance.currentDay + 1) % 20, Instance.currentHour, Instance.currentMinute, 0);
	...
	void Update () {
		...
		var oneHour = 1 / 24.0f;
		currentHour = (int)(currentTime / oneHour);
		var oneMinute = oneHour / 60;
		currentMinute = (int)((currentTime % oneHour) / oneMinute);
        if (currentTime >= 1) {
	...
	public void SkipMinutes(int minutes)
	{
        var oneHour = 1 / 24.0f;
        var oneMinute = oneHour / 60;
		currentTime += minutes * oneMinute;
    }

    public void SkipTicks(float ticks)
    {
        currentTime += ticks;
    }
	...
	void OnGUI()
	{
		...
		if (showUI) {
			DateTimeUI.Instance.SetDateTime(Now);
        }
	}

	public static DayAndNightControl Instance { get; private set; }
}
